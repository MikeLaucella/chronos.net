...
class SegformerBolt2(LightningModule):
    ...
    def forward(self, inp: dict) -> torch.Tensor:
        ...
        with torch.amp.autocast(self.device.type):
            y_modern = self.segformer.full(img1)

            out = self._metrics.score(mask, y_modern.logits)
            ce_loss = out['loss']
            
            if self.current_epoch > 5:
                y_hist = self.segformer.full(img2)
                #feat_modern = y_modern.hidden_states
                #feat_hist = y_hist.hidden_states

                loss_const = proxy_loss = F.kl_div(
                    F.log_softmax(y_hist.logits, dim=1),
                    F.softmax(y_modern.logits.detach(), dim=1), # Detach so we don't backprop through the modern 'target'
                    reduction='batchmean'
                )
                #loss_const = self._structural_consistency_loss(feat_modern, feat_hist, stages=[2, 3])
            else:
                loss_const = torch.tensor(0.0, device=img1.device)

        # Total Loss
        # lambda_c is usually small (e.g., 0.1) to prevent the style 
        # from 'overwhelming' the actual class learning
        total_loss = ce_loss + (0.05 * loss_const)
        out['ce_loss'] = ce_loss
        out['loss'] = total_loss
    ...
    def _on_end(self, batch, type: str):
        ...
        with torch.no_grad():
            ...
            logits_naip = self.segformer(naip)
            logits_eros = self.segformer(eros)
    ...
    def configure_optimizers(self) -> tuple:
        ...
        pg = param_groups_seg(self.segformer, self.lr)
